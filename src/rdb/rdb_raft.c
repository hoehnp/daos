...
rdb_raft_exec_unpack_io(struct dc_obj_enum_unpack_io *io, void *arg)
{
	...
	/* We should consider using one VOS local TX for multiple updates. */
...
static int
rdb_raft_update_node(struct rdb *db, uint64_t index, raft_entry_t *entry, struct dtx_handle *vtx)
{
	d_rank_list_t  *replicas;
			  ...
			  struct dtx_handle *vtx)
{
	...
	if (entry->data.len > 0) {
		...
		if (rc != 0) {
			D_ERROR(DF_DB": failed to look up entry "DF_U64" data: %d\n", DP_DB(db),
				index, rc);
			return rc;
		}
		      ...
		      raft_index_t index, int *n_entries)
{
	 ...
	 * one entry encounters an error, prior entries in the batch will not
	 * be rolled back. (If we support batching VOS TXs, for instance, in
	 * the TX group manner, then we can optimize this process further.)
	 */
	for (i = 0; i < *n_entries; ++i) {
		struct dtx_handle      *vtx;
		int			rc_tmp;

		rc = vos_local_tx_begin(db->d_pool, &vtx);
		if (rc != 0) {
			D_ERROR(DF_DB": failed to begin VOS TX for entry %ld: "DF_RC"\n",
				DP_DB(db), index, DP_RC(rc));
			break;
		}

		rc = rdb_raft_log_offer_single(db, &entries[i], index + i, vtx);

		rc_tmp = vos_local_tx_end(vtx, rc);
		if (rc_tmp != 0) {
			D_ERROR(DF_DB": failed to end VOS TX for entry %ld: "DF_RC"\n", DP_DB(db),
				index, DP_RC(rc_tmp));
			if (rc == 0)
				rc = rc_tmp;
		}
